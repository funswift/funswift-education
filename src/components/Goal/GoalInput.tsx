...
export default function GoalInput() {
  ...
  const handleComplete = () => {
    ...
    const wBed = parseTimeToDate(weekend[0]);
    const wWake = parseTimeToDate(weekend[1]);
    ...
    const payload = {
      ...
      // weekend
      weekendBedTimeGoal: wBed ? wBed.toISOString() : null,
      weekendWakeUpTimeGoal: wWake ? wWake.toISOString() : null,
    ...
    try {
      ...
      console.log("Saved goal (weekday+weekend):", payload);
    } catch (e) {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh", padding: "2rem" }}>
      <div style={{ width: "min(820px, 95vw)", border: "2px solid #ccc", borderRadius: 12, padding: "1.25rem", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
        <h2 style={{ margin: 0, marginBottom: 12 }}>目標時刻を入力（平日 / 週末）</h2>

        <div style={{ display: "flex", gap: 24 }}>
          {/* Weekday column */}
          <div style={{ flex: 1 }}>
            <h3 style={{ marginTop: 0, marginBottom: 8 }}>平日</h3>
            {weekday.map((t, i) => {
              ...
              return (
                <div key={i} style={{ marginBottom: 10 }}>
                  ...
                  {isDuration ? (
                    <select id={`weekday-input-${i}`} value={t} onChange={(e) => handleChange("weekday", i, e.target.value)} style={{ width: "100%", padding: ".5rem .75rem", fontSize: "1rem", borderRadius: 8, border: "1px solid #ddd" }}>
                      <option value="">選択してください</option>
                      {slots.map(m => <option key={m} value={String(m)}>{m} 分</option>)}
                    </select>
                  ) : (
                    <input id={`weekday-input-${i}`} type="time" value={t} onChange={(e) => handleChange("weekday", i, e.target.value)} style={{ width: "100%", padding: ".5rem .75rem", fontSize: "1rem", borderRadius: 8, border: "1px solid #ddd" }} />
                  )}
                </div>
              );
            })}
          ...
          {/* Weekend column */}
          <div style={{ flex: 1 }}>
            <h3 style={{ marginTop: 0, marginBottom: 8 }}>週末 / 祝日</h3>
            {weekend.map((t, i) => {
              const isDuration = i === 2 || i === 3;
              return (
                <div key={i} style={{ marginBottom: 10 }}>
                  ...
                  {isDuration ? (
                    <select id={`weekend-input-${i}`} value={t} onChange={(e) => handleChange("weekend", i, e.target.value)} style={{ width: "100%", padding: ".5rem .75rem", fontSize: "1rem", borderRadius: 8, border: "1px solid #ddd" }}>
                      <option value="">選択してください</option>
                      {slots.map(m => <option key={m} value={String(m)}>{m} 分</option>)}
                    </select>
                  ) : (
                    <input id={`weekend-input-${i}`} type="time" value={t} onChange={(e) => handleChange("weekend", i, e.target.value)} style={{ width: "100%", padding: ".5rem .75rem", fontSize: "1rem", borderRadius: 8, border: "1px solid #ddd" }} />
                  )}
                </div>
              );
            })}
          ...
          <div style={{ display: "flex", gap: 24, marginTop: 8 }}>
            <div style={{ flex: 1 }}>
              ...
              <ul style={{ margin: "6px 0 0 16px", padding: 0 }}>
                {weekday.map((t, i) => <li key={i} style={{ listStyle: "decimal" }}>{`${labels[i]}: ${t || "未選択"}`}</li>)}
              </ul>
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600 }}>週末 / 祝日</div>
              <ul style={{ margin: "6px 0 0 16px", padding: 0 }}>
                {weekend.map((t, i) => <li key={i} style={{ listStyle: "decimal" }}>{`${labels[i]}: ${t || "未選択"}`}</li>)}
              </ul>
            </div>
          </div>
